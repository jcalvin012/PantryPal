...
const STORAGE_RULES = [
  ...
  { match: ['canned', 'can of', 'crackers', 'biscuits', 'cookies', 'instant noodles'], category: 'Canned', freshLocation: 'Pantry', frozenLocation: 'Pantry', freshDays: null, frozenMonths: null, reason: 'Shelf-stable packaged foods belong in a cool, dry pantry unless the package says otherwise.' },
...
export function getStorageRecommendation({ name, condition = 'Fresh', category = 'Other' } = {}) {
  ...
  if (normalizedCondition === 'Frozen') {
    return { location: rule?.frozenLocation || (rule?.category === 'Meat' || rule?.category === 'Seafood' ? 'Freezer' : 'Freezer'), reason: rule?.reason || 'The item is already frozen; keep it frozen for storage.', matched: Boolean(rule), category: rule?.category || category }
  }
...
export function estimateExpiry({ name, condition = 'Fresh', location = '', purchaseDate, today = new Date().toISOString().slice(0, 10), category = 'Other' } = {}) {
  ...
  if (!rule) return { expiryDate: null, estimated: false, basis: 'No matching storage rule', reason: 'PantryPal could not confidently estimate a storage period for this food.' }
  ...
  if ((normalizedCondition === 'Fresh' || normalizedCondition === 'Frozen') && targetLocation === 'freezer' && rule.frozenMonths) {
